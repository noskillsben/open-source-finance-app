...
**A one-off income is not a named pay.** A bonus, a tax refund or a single gig payment is a future-dated income transaction with no link, dated on the day you expect it and edited when it lands — future-dated rows are already the plan (General concepts). A named pay exists so that goals can bind to it and count toward its next payday, and nothing binds to a pay that comes once. So every named pay repeats, and there is no one-off cadence — not on streams, and not on goals, where "once" would only be a second way to say a Target with a date.
