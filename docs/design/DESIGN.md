...
- **Nothing posts itself.** Every recurring bill offers a one-tap "record now" on its own row on Categories, naming the due date it would pay ("Record Oct 1" — its earliest unpaid due date, Goals → Paying a bill); it opens the transaction form pre-filled and linked to that bill and due date, and the user saves it. A named pay offers **Record** on Pay, which opens the pay screen for that payday. Those two surfaces and no others — never a separate "what's due" list. The app never writes a transaction on its own — the single exception is the opening-balance adjustment it maintains (Balance checks). Silent auto-posting is declined deliberately — a transaction is what has happened, a goal is what hasn't, and there is no third state.
...
- **Recurring bill** — amount, cadence (monthly, quarterly, yearly, every N weeks) and a **first due date, which is required**. Its due dates are the first due date stepped forward by the cadence; no cycle exists before it. The envelope refills after each payment, and the bill knows which of its due dates were paid, because a payment says so (Paying a bill, below).
...
**On the category's row on Categories** a category with a goal shows a goal row under it: the goal's name, progress as "$132.82 of $1,048.00", the due date (for a recurring bill, its status line and "record now" — Paying a bill, below), the per-period amount, and an amount box with Add / Withdraw — a quick earmark move between "ready to assign" and this category. The category row itself shows assigned, spent, available, and the pool pill ("+$170.00 available if overspent"). Progress and available are the same number read two ways; there is no separate goal pot.

**Paying a bill.** A transaction can say which recurring bill it paid and which of that bill's due dates it paid, on its header (Transactions). The due date is stated, never worked out from the payment date, because a bill payment is routinely early or late: rent paid on Oct 3 paid the Oct 1 rent. This is the same question the named-pay link answers — which plan did this fulfil — asked of a bill.

- **One linked payment marks that due date paid**, whatever its amount. A bill's amount is an estimate (hydro is $112 one month and $134 the next), so an amount rule would leave cheap months unpaid forever. Two payments against one due date — rent in two transfers — both link to it. The goal row shows what was paid beside what was expected ("Oct 1 paid · $112.40 of $120.00"), so a genuine half-payment stays visible.
- **The link is always stated, never inferred.** "Record now" sets it. In the Ledger form, a category line on a category with a live recurring bill offers "this pays *Rent*, due Oct 1" (the earliest unpaid due date), and the user confirms it, picks another of the bill's due dates, or leaves the payment unlinked. Spending on the category with no link never marks a bill paid.
- **"Record now" pre-fills** the bill's category and expected amount, the link, the picker date — the date money moved; paying before the due date is normal, and the due date lives on the link — and the payee and account of the last payment linked to this bill, read from the ledger and never stored (empty on a bill's first payment).
- **The status line** on the goal row reads *overdue since Oct 1* once the picker is past the bill's due date; *due Oct 1 · not paid* when the due date falls on or before its bound pay's next payday (or is the picker date, for a bill bound to no pay); *next due Nov 1* otherwise. The pay screen's Bills block reads the same due date, so the app has one definition of due.
- **Editing a bill's cadence or first due date restarts its cycles** at the new first due date. Links to the old dates stay on their transactions as history and mark nothing paid.
- **Named pays need none of this.** Their link already says which payday was recorded, because the pay screen dates the transaction on the payday; paydays with no transaction are what Pay history shows.
...
Three different questions, three different answers, none duplicated: *who paid me* is the payee; *what kind of income* is the category (Salary income, Gig income); *which plan did this fulfil* is the named-pay or bill link on the header.
...
6. **Bills** — every Recurring-bill goal bound to this pay, each at its earliest unpaid due date (Goals → Paying a bill), so an overdue bill is listed too. Those due before the next payday come first, pre-filled with what is still missing for the full amount; those due later follow, pre-filled with their instalment (what is still missing ÷ paydays left — the $1,200 quarterly bill with $400 saved and two paydays left shows $400). Context line: category · due date · expected.
...
### History replay — end-to-end check on real-shaped data

A **history replay** plays a recorded money history into a fresh database through the same HTTP calls the screens make, so the whole app is exercised the way a person would use it: accounts opened, pays recorded and re-opened on the pay screen's two calls, bills paid through their links, balance checks stated. It is a development tool, not a feature, and has nothing to do with "audit replay" (Founding decisions) — the app still keeps no event log.

- **A scenario is data, the engine is code.** A scenario is one file: a setup (accounts, payees, domains, categories, named pays, goals) and a list of dated events (transaction, move, pay, re-open pay, balance check), every entity addressed **by name**, never by id, in the order they were entered. The engine resolves names against the database it runs on and replays events up to a date, or stops before an event kind so a person can do it by hand on its screen and carry on.
- **It checks against the running app, never against a copy of its rules.** The pass/fail is what the app itself reports afterwards: every balance check sees the balance the source recorded before its adjustment, every final balance equals the source's, and the integrity check is empty. A second implementation of the arithmetic in the tool would be a second mechanism that drifts; a converter may check its own output offline while it is being written, and that check is never committed.
- **It only calls the public API.** No direct SQL, no endpoint of its own, no "replay mode" in the backend. If a scenario cannot be expressed through the screens' calls, that is a finding about the app, not a reason to add a back door.
- **It is not an import.** Moving data in from a bank or another app is a separate question with its own design (payee aliases wait for it). A converter from another app's export is a one-off script beside the scenario it produces.
- **Personal data is never committed.** The engine and one small fictional sample scenario live in the repo; real scenarios, converters written against a real export, and the exports themselves live only in the gitignored folder `CLAUDE.md` names. The first real scenario is Ben's old-app history, Aug 6 – Sep 25 2026, converted from the old app's export; checked offline while it was written, it reproduces every balance check and final balance to the cent, and has not yet been run against the app.
...
- 2026-09-24 — Every recurring bill now says whether it was paid: its row on Categories reads *due Oct 1 · not paid*, *overdue since Oct 1* or *next due Nov 1*, and "Record Oct 1" pays that exact due date. Mechanism: a transaction can carry which bill it paid and which of its due dates (`goal_id`, `goal_due_on` on the header) — the named-pay link's question, "which plan did this fulfil", extended to bills rather than a second way to ask it. The link is set by "record now" or confirmed in the Ledger form, never inferred from spending on the category; one linked payment marks a due date paid, and the row shows paid against expected. A bill's due date is the earliest unpaid one, stepped from a first due date that is now required. "Record now" pre-fills the picker date, and the payee and account of the bill's last linked payment. Named pays are unchanged — Record / Re-open already knows. **Deletes:** the roll-forward that made a bill impossible to be overdue (an unpaid bill became next month's bill the day after it was due); the optional date on recurring bills; "that is due" as the condition for record now; the named-pay half of #26, already built. Schema: additive — two nullable columns on `transaction`; migration — the first due date becomes required on recurring bills, and any dateless bill is given one by hand before it runs. User guide: Categories gains the bill status line and "record now"; Ledger gains "mark which bill and due date a payment paid". Answers #130.
- 2026-09-25 — The app can be checked end to end by replaying a recorded history into a fresh database: a scenario file of named entities and dated events is played through the same HTTP calls the screens make, and passes when the app's own balance checks, final balances and integrity check agree with the source. Mechanism: a development tool beside the app — engine and a fictional sample in the repo, real scenarios and converters only in a gitignored folder; it calls the public API and nothing else, and verifies against the running app rather than a re-implementation of its rules. **Deletes:** nothing — it adds no app mechanism, and it is explicitly not an import. Schema: nothing. User guide: no user-facing change. Convention (private data path, never commit personal data) lands in `CLAUDE.md`.
