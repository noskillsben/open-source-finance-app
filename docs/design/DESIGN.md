...
- Whether ready to assign should exclude overspent categories, the way YNAB's to-be-budgeted does, or keep reading them back as the plain difference does today and surface the overspend instead. Excluding them puts a second `max()` into the one formula the 2026-09-17 decision took a `max()` out of. Settle it when the earmark ledger (#17) is scoped, not after it is built.
