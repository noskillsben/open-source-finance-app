...
- **Non-ledger rows are archived, never deleted.** Every non-ledger row carries `created_on` and a nullable `archived_on`. It is shown on a given picker date if `created_on ≤ date` and (`archived_on` is null or `date < archived_on`). Archiving is the only "delete"; it defaults to the picker date. Rules:
  ...
  - Archiving an account or category with a non-zero balance as of the archive date is a warning, not a block. Archived rows are ignored by the unique-name rule, so a mistyped "Grocries" can be archived and "Groceries" created; unarchiving refuses (planning surface) if the name is now taken.
