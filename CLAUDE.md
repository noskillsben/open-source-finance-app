...
- Settings never rewrite history: a transaction's on-budget cents are computed at write time and stored on the line; changing a boundary category, pool, link or an account's budget side affects later writes only. On-budget cents are the line's own cents when its account was on-budget at write time, 0 otherwise — never the floor, never a running balance, never write order (DESIGN.md § Transactions → Invariant).
...
- Null means unknown, never zero, on every rate or term field — the single exception is `credit_limit_cents` pre-filled to 0 for the no-credit account types (DESIGN.md § Debt terms).
...
- **Tests never touch the app's database.** The suite runs against a throwaway database it creates and drops itself; `conftest.py` aborts the whole session if the resolved test URL is the app's. Nothing in `backend/tests/` may commit to, or do DDL on, the running app's data — not even with a cleanup in a `finally`.
- **Every revision gets a populated-database migration test, written with it, and it keeps running forever.** The test takes a throwaway database to that revision's `down_revision`, loads `backend/tests/fixtures/<revision>.sql` — raw SQL, because the ORM models describe *head* and cannot populate an older schema — then runs `alembic upgrade head` and asserts the data survived, any backfill landed the values the revision promised, and a second upgrade is a no-op. One fixture per revision, written once; the harness is parametrised over revisions so nobody has to remember to run it.
- **`downgrade()` is best-effort and untested.** The recovery path is the pre-migrate `pg_dump` the entrypoint already takes, which is what you would actually reach for on the Pi. Autogenerated downgrades are kept for stepping back a revision on a development database you don't mind breaking; never rely on one, and never write a migration whose forward path depends on its downgrade working.
