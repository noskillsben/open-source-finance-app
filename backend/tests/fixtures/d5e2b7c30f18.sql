-- At down_revision c4d8a1f65e92 there is no earmark_line. This revision only adds that table,
-- so a chequing account, one category and a spend against it must come through untouched, and
-- the category's balance (transaction category lines only, until earmark lines exist) must not move.
