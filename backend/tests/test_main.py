...
def test_delete_transaction_unlinks_but_keeps_its_pay_batch(db_session):
    ...
    separate, offered action — saving it empty — so deleting the transaction must never fail on
    that reference, and the batch's money must survive untouched.
    """
    from app.services.earmarks import pay_batch_lines, replace_pay_batch
    ...
    replace_pay_batch(
        db_session, txn,
        [{"category_id": salary.id, "cents": -2_400_00}, {"category_id": groceries.id, "cents": 500_00}],
    )

    resp = _delete(db_session, txn.id)

    assert resp.status_code == 204
    assert pay_batch_lines(db_session, txn.id) == []  # unlinked, not gone
